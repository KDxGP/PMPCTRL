...
class PmpctrlAPI(FastAPI):
    ...
    def get_root(self) -> dict:
        ...
        if self._control_data.get_time_utc_session_start() is not None:
            start_session = self._control_data.get_time_utc_session_start().isoformat()
        if self._control_data.get_last_session_duration() is not None:
            last_session_duration = self._control_data.get_last_session_duration()
        return {
            ...
            'time_utc_session_start' : start_session,
        ...
        if target_new.target is not None:
            ...
            if pressure_min <= target_new.target <= pressure_max:
                ...
                error = ErrorMessage(
                    ...
                    detail = (
                        f'The new pressure target is not within range of '
                        f'min={pressure_min} to max={pressure_max}'
                    )
                )
                raise ApiError(error)

        # defined here because used twice:
        error = ErrorMessage(
                    status = 400,
                    title = 'Pressure tolerances must be defined as >= 0',
                    detail = 'e.g. Set as 5.6, not -5.6'
                )
    ...
    def put_pressure_setpoint(self, setpoint: Setpoint | None = None):
        ...
        if setpoint is None:
            ...
            if setpoint.setpoint is not None:
                pressure_min = self._control_data.get_pressure_min()
                pressure_max = self._control_data.get_pressure_max()
                if pressure_min <= setpoint.setpoint <= pressure_max:
                    self._control_data.set_pressure_setpoint(setpoint.setpoint)
                else:
                    error = ErrorMessage(
                        status = 400,
                        title = 'Setpoint out of range',
                        detail = (
                            f'The new setpoint is not within range of '
                            f'min={pressure_min} to max={pressure_max}'
                        )
                    )
                    raise ApiError(error)
